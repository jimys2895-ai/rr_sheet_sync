/**
 * RoseRocket outbound sheet — force refresh.
 *
 ...
 *
 * One-time setup (client / sheet owner):
...
function setupRoseRocketWebhook() {
  PropertiesService.getScriptProperties().setProperties({
    WEBHOOK_URL: 'https://YOUR-SERVICE.onrender.com/refresh/outbound',
...
function pinRefreshTab() {
  ...
  sheet.getRange('A2').setFormula('=HYPERLINK("' + linkUrl + '", "Force refresh outbound sheet")');
...
function doGet(e) {
  ...
  if (showUi) {
    ...
    const detail = result.ok
      ? (result.message || 'Outbound sheet updated.')
...
function runWebhookRefresh() {
  ...
  try {
    ...
    if (code === 200) {
      return { ok: true, message: body.message || 'Outbound sheet refreshed.' };
    }
    if (code === 409) {
